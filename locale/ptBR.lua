local L = select(2, ...)[3]('ptBR')

-- Categories
L['Artifact Power'] = ARTIFACT_POWER
L['Collections'] = COLLECTIONS
L['Consumables'] = BAG_FILTER_CONSUMABLES
L['Equipment'] = BAG_FILTER_EQUIPMENT
L['Equipment Sets'] = 'Conjunto de Equipamentos'
L['World Event'] = BATTLE_PET_SOURCE_7
L['Erroneous'] = 'Errôneo'
L['Inventory'] = INVENTORY_TOOLTIP
L['Junk'] = BAG_FILTER_JUNK
L['New Items'] = 'Novos Itens'
L['Professions'] = TRADE_SKILLS
L['Quests Items'] = AUCTION_CATEGORY_QUEST_ITEMS
L['Reagent Bank'] = REAGENT_BANK
L['Teleporters'] = 'Teleportes'
L['Trade Goods'] = AUCTION_CATEGORY_TRADE_GOODS

-- Container button tooltips
L['Mark items as known'] = 'Marcar itens como conhecidos'
L['Toggle auto-deposit'] = 'Ativar auto-deposito'
L['Toggle bag slots'] = 'Mostrar Bolsas'
L['Toggle auto-vendoring'] = 'Ativar venda automática' -- needs translation (done)
L['Restack'] = 'Reorganizar'
L['Restack Bank'] = 'Reorganizar Banco'
L['Restack Reagent Bank'] = 'Reorganizar Banco de Reagentes'
L['Deposit All Reagents'] = REAGENTBANK_DEPOSIT
L['Lock'] = LOCK
L['Unlock'] = UNLOCK

-- Misc messages
L['Purchase'] = PURCHASE
L['Equip Container'] = EQUIP_CONTAINER

-- Error messages
L['A layout already exists (%s)'] = 'Um layout já existe (%s)'
L['Override for "%s" already exists'] = 'Substituir por "%s" já existe'

-- Config
L['Here you can toggle categories or even create new ones!'] = 'Aqui você pode ativar/desativar categorias ou até criar novas!'
L['Modifier to show bank when toggling Backpack'] = 'Modificador para mostrar o banco quando abrir a Mochila'
